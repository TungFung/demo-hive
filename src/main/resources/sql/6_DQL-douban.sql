...
STORED AS TEXTFILE
LOCATION "/user/hadoop-twq/hive-course/links";
...
select m.movieid, m.moviename, initialreleasedatemap,map_keys(initialreleasedatemap) from movie m;
--4.3、类型转换函数
select cast(movieid as int) as int_movieid, moviename from movie limit 10;
--4.4、时间函数
--查询出在美国上映的时间的月份和日
select year(initialreleasedatemap["美国"]), month(initialreleasedatemap["美国"]), day(initialreleasedatemap["美国"]) from movie limit 10;
--4.5、字符串函数
select commentscore,format_number(commentscore, 4) from movie limit 10;
--字符串拼接
...
select ctgy from movie lateral view explode(category) category as ctgy;
