...
insert overwrite table sensor_managed_partition partition(year=2018,month,day)
select p.id,p.event_id,p.event_type,p.part_name,p.part_number,p.version,p.payload,month,day from sensor_managed_partition p;



--(四) Writing data into the filesystem from queries
--数据量大的话则写到HDFS的文件中
INSERT OVERWRITE DIRECTORY '/hive-data/sensor_managed_partition'
ROW FORMAT DELIMITED 
FIELDS TERMINATED BY '\t'
COLLECTION ITEMS TERMINATED BY ','
MAP KEYS TERMINATED BY ':'
STORED AS TEXTFILE
SELECT * FROM sensor_managed_partition;
